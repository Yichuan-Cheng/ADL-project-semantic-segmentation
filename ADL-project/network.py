...
class DAPPM(nn.Module):
    ...
    def forward(self, x):
        ...
        # print([height, width],self.scale4(x).shape, x_list[3].shape)
...
class DualResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # [B,planes*4,H/16,W/16]
        layers.append(x)

        x_ = self.layer3_(self.relu(layers[1]))
        # [B,highres_planes,H/8,W/8]
        ...
        # [B,2*highres_planes,H/8,W/8]
        print(self.spp(self.layer5(self.relu(x))).shape)
        x = F.interpolate(
