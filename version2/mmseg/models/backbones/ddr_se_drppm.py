...
from mmseg.models.utils import DAPPM, BasicBlock, Bottleneck, resize, DRPPM
    ...
    def __init__(self,
        ...
        for i in range(3):
            self.context_branch_layers.append(
                nn.Sequential(SELayer(channels * 2**(i + 1)), self._make_layer(
        ...
        for i in range(3):
            self.spatial_branch_layers.append(
                nn.Sequential(SELayer(channels * 2), self._make_layer(
                    block=BasicBlock if i < 2 else Bottleneck,
                    inplanes=channels * 2,
                    planes=channels * 2,
                    num_blocks=2 if i < 2 else 1,
